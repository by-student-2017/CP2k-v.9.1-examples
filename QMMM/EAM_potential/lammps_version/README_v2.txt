...
Note: In the mixed case, first try changing fe. If that doesn't work, try using Tersoff and similar mixing rules to get a better fit. If that doesn't work, try fitting by adding constraints based on the relationships shown above. In the case of high entropy alloys, they basically have crystal structures of FCC, BCC, and HCP, so it would be best to make the shape suitable for that crystal. The parameters of many pure metals reproduce FCC, BCC, and HCP quite well, so the results are not expected to be too bad.
